...
class WorkflowTaskService {
    ...
    taskInstance.task.approverRoles.each {
      ...
      if(role) {
        ...
        role.subjects.each { sb -> 
          def s = aaf.base.identity.Subject.get(sb.id)
  ...
  void messageApprovalRequest(def sb, TaskInstance taskInstance) {
    ...
    def contents = groovyPageRenderer.render(view: '/templates/branding/mail/_workflow_requestapproval', plugin: "base", model: [taskInstance: taskInstance, waListLink:waListLink])
    def email_msg = groovyPageRenderer.render(view: '/layouts/email')
    email_msg = email_msg.replace("###HTML EMAIL CONTENT###", contents)
    mailService.sendMail {
      to sb.email   
      subject messageSource.getMessage('branding.workflow.mail.requestapproval.subject', args, 'branding.workflow.mail.requestapproval.subject', new Locale("EN"))
      body(email_msg)
    }
  }
  ...
  def executeScript(def scriptName, def env) {
    ...
    if(workflowScript) {
      def _log = LogFactory.getLog("domains.aaf.base.workflow.script.$scriptName")
