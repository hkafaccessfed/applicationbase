...
import aaf.base.ExtendedGrailsTemplateGenerator
 ...
 * @since 0.4
 *
 * Modified for AAF Application Base - Bradley Beddoes
 */
...
warning = "\n\n**** This code is not production ready. Please verify all tests, restrict data binding and perform other quality checks before commiting to your project ****\n"
...
target(generateForOne: "Generates controllers and views for only one domain class.") {
    ...
    if (domainClass) {
        ...
        event("StatusFinal", ["Finished generation for domain class ${domainClass.fullName} $warning"])
    }
    else {
...
target(uberGenerate: "Generates controllers and views for all domain classes.") {
    ...
    if (domainClasses) {
        ...
        event("StatusFinal", ["Finished generation for domain classes $warning"])
    }
    else {
...
def generateForDomainClass(domainClass) {
    TemplateGenerator = classLoader.loadClass('aaf.base.ExtendedGrailsTemplateGenerator')
    def templateGenerator = TemplateGenerator.newInstance(classLoader)
