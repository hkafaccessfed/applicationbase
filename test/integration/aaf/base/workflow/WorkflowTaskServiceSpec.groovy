...
class WorkflowTaskServiceSpec extends IntegrationSpec {
  ...
  def setup() {
    ...
    subject = new Subject(principal:'testsubject', cn:'test subject', email:'test@testdomain.com', sharedToken:'1234sharedToken').save()
  ...
  def "Validate first task in minimal process requires approval when process is run"() {
    ...
    (initiated, processInstance) = workflowProcessService.initiate('Minimal Test Process', "Approving XYZ Widget", ProcessPriority.LOW, ['agent':subject, 'TEST_VAR':'VALUE_1', 'TEST_VAR2':'VALUE_2', 'TEST_VAR3':'VALUE_3'])
    ...
    message.subject == 'branding.workflow.mail.requestapproval.subject'
  }
  ...
  def "Validate first task in minimal process requires approval when process is run and approval is provided to subjects in role1, also validates correct variable substition to workflow script"() {
    ...
    (initiated, processInstance) = workflowProcessService.initiate('Minimal Test Process', "Approving XYZ Widget", ProcessPriority.LOW, ['agent':subject, 'TEST_VAR':'role1', 'TEST_VAR2':'VALUE_2', 'TEST_VAR3':'VALUE_3'])
    
    def role = new Role(name: 'role1')
    def subject2 = new Subject(principal:'testsubject2', cn:'test subject2', email:'test2@testdomain.com', sharedToken:'5678sharedToken2').save()
    ...
    def message = greenMail.getReceivedMessages()[0]
    'branding.workflow.mail.requestapproval.subject' == message.subject
    ...
    'branding.workflow.mail.requestapproval.subject' == message2.subject
  ...
  def "Validate first task in minimal process executes when process is run and task approved"() {
    ...
    minimalDefinition = new File('test/data/minimal.pr').getText()
    workflowProcessService.create(minimalDefinition)
    def env = ['agent':subject, 'TEST_VAR':'VALUE_1', 'TEST_VAR2':'VALUE_2', 'TEST_VAR3':'VALUE_3']
    def initiated, processInstance
  ...
  def "Validate third and forth task in minimal process are started (forth executed no approver needed) after full completion of first 2"() {     
    ...
    workflowProcessService.create(minimalDefinition)
    def initiated, processInstance
    (initiated, processInstance) = workflowProcessService.initiate('Minimal Test Process', "Approving XYZ Widget", ProcessPriority.LOW, ['agent':subject, 'TEST_VAR':'VALUE_1', 'TEST_VAR2':'VALUE_2', 'TEST_VAR3':'VALUE_3', 'TEST_VAR4':'VALUE_4', 'TEST_VAR5':'VALUE_5'])
  ...
  def "Ensure all task instances that a subject can approve are returned when requested"() {
    setup:
    def subject2 = new Subject(principal:'testsubject2', cn:'test subject', email:'test2@testdomain.com', sharedToken:'5678sharedToken').save()
